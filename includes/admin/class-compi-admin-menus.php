...
class Compi_Admin_Menus {
	...
	public function admin_menu() {
		...
		add_menu_page( Compi()->title_name, Compi()->menu_name, Compi()->manage_plugin, COMPI_PAGE, array( $this, 'compi_page' ), null, '25.5' );

		$settings_menu = isset( Compi()->full_settings_menu ) ? Compi()->full_settings_menu : '';

		if ( $settings_menu == '' || $settings_menu == 'no' ) {
			$settings_page = add_submenu_page( COMPI_PAGE, sprintf( __( '%s Settings', COMPI_TEXT_DOMAIN ), Compi()->title_name ), __( 'Settings', COMPI_TEXT_DOMAIN ) , Compi()->manage_plugin, COMPI_PAGE . '-settings', array( $this, 'settings_page' ) );
		}
		else{
		...
		add_submenu_page( COMPI_PAGE, sprintf( __( '%s Status', COMPI_TEXT_DOMAIN ), Compi()->title_name ), __( 'System Status', COMPI_TEXT_DOMAIN ), Compi()->manage_plugin, COMPI_PAGE . '-status', array( $this, 'status_page' ) );

		add_submenu_page( COMPI_PAGE, sprintf( __( '%s Tools', COMPI_TEXT_DOMAIN ), Compi()->title_name ), __( 'Tools', COMPI_TEXT_DOMAIN ), Compi()->manage_plugin, COMPI_PAGE . '-status&tab=tools', array( $this, 'status_page' ) );
