...
if ( ! class_exists( 'Compi_Transifex_Stats' ) ) {

	class Compi_Transifex_Stats {
		...
		public function __construct( ) {
			$this->project_slug  = Compi()->transifex_project_slug;
			$this->resource_slug = Compi()->transifex_resources_slug;
			$this->cache_time    = 3600;
			$this->api = new Compi_Transifex_API( $this->cache_time );
		} // END __construct()
		...
		public function maybe_display_error( $response ) {
			...
			if ( ! $response ) {
				$error = __('No results', COMPI_TEXT_DOMAIN );
			}
		 ...
		 * @since  1.0.0
		 * @access public
		 * @filter compi_transifex_contributors
		 * @return array API result
		 */
		public function display_contributors() {
			...
			$contributors = apply_filters( 'compi_transifex_contributors', array_unique( $contributors ) );
		...
		public function display_translations_progress() {
			...
			$languages = apply_filters( 'compi_transifex_stats', $languages, $project );
				...
				<li class="clearfix">
					...
					<div class="statbar">
						...
						<div class="go_translate">
							<a target="_blank" href="https://www.transifex.com/projects/p/<?php echo Compi()->transifex_project_slug; ?>/translate/#<?php echo $language_code; ?>"><?php _e( 'Translate', COMPI_TEXT_DOMAIN ); ?></a>
						</div>
					</div>
				</li>
				<?php
