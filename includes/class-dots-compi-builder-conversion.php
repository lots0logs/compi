...
class Dots_Compi_Conversion_Util {
	...
	public function __construct() {
		...
		$this->map              = Dots_Compi_EBuilder_Mapping::$mapping;
...
class Dots_Compi_EBuilder_Element {
	...
	public function in_range( $val, $min, $max ) {

		return ( $val >= $min && $val <= $max );
	}

	public function maybe_add_column_wrapper() {

		if ( true === $this->parent->is_row && false === $this->is_column ) {
			$siblings = $this->parent->get_len();
			if ( 2 === $siblings && array_key_exists( $this->old_attrs, 'width' ) ) {
...
class Dots_Compi_EBuilder_Shortcode extends Dots_Compi_EBuilder_Element {
	public function __construct( &$parent, $index, $old_as_string, $slug, $attrs_string, $contents ) {
	...
	private function process_children() {
		...
		if ( is_array( $nested_tags ) && ! empty( $nested_tags ) ) {
			foreach ( $nested_tags[2] as $nested_tag => $slug ) {
				$old_as_string = isset( $nested_tags[0][ $index ] ) ? $nested_tags[0][ $index ] : '';
				$attrs_string  = isset( $nested_tags[3][ $index ] ) ? $nested_tags[3][ $index ] : '';
				$contents      = isset( $nested_tags[5][ $index ] ) ? $nested_tags[5][ $index ] : '';

				$shortcode = new Dots_Compi_EBuilder_Shortcode( $this, $index, $old_as_string, $slug, $attrs_string, $contents );

				$this->add_child( $shortcode );
...
class Dots_Compi_EBuilder_Mapping {
