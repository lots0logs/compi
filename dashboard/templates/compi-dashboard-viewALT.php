...
echo '<div id="dots_admin_wrapper" class="dots_admin">
...
if ( isset( $dash_tabs ) ) {
	...
	foreach ( $dash_tabs as $key => $value ) {
		if ( isset( $value['title'] ) ) {
			printf( '<li class="%3$s"><a href="#fixed-tab-%1$s" aria-controls="fixed-tab-%1$s" role="tab" data-toggle="tab">%2$s</a>',
			        esc_attr( $key ),
			        $value['title'],
...
if ( isset( $dash_tabs ) ) {
	...
	foreach ( $dash_tabs as $key => $value ) {
		...
		if ( $key !== 'header' ) {
			printf( '<div  role="tabpanel" class="tab-panel%2$s" id="fixed-tab-%1$s">
					...
					switch ( $option['type'] ) {
						...
						case 'card_start' :
							printf(
								'<div class="col-sm-4">
                            <div class="card">
                                <div class="card-header bgm-cyan%2$s">
						...
						case 'card_option' :
							printf(
								'<div class="toggle-switch">
                                        <label for="%2$s" class="ts-label"></label>
                                        <input id="%2$s" type="checkbox" hidden="hidden" %3$s>
                                        <label for="%2$s" class="ts-helper"></label>
                                    </div>
                                       </ul>
                                </div>
                                <div class="card-body card-padding">%1$s</div>
                                ',
