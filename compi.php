...
if ( !class_exists( 'Compi' ) ) {
	...
	final class Compi {
		...
		/**
		 * The minimum WordPress version Compi requires.
		 *
		 * @since  1.0.0
		 * @access public
		 * @var    string
		 ...
		 * @var    string
		 */
		public $memory_limit = '640'; // returns 32 MB
		 ...
		 * @var    string
		 */
		public $web_url = "http://wpdots.com/plugins/compi";

		/**
		 * Compi documentation URI.
		 *
		 * @since  1.0.0
		 * @access public
		 * @var    string
		 */
		public $doc_url = "http://github.com/wpdots/compi/wiki/";
		 ...
		 * @since   1.0.0
		 * @example https://wordpress.org/plugins/compi
		 * @access  public
		 * @var     string
		 */
		public $wp_plugin_url = "https://wordpress.org/plugins/compi";
		 ...
		 *          plugin slug given for your wordpress repository
		 * @since   1.0.0
		 * @example https://wordpress.org/support/plugin/compi
		 * @access  public
		 * @var     string
		 */
		public $wp_plugin_support_url = "https://wordpress.org/support/plugin/compi";
		 ...
		 *          plugin slug given for your wordpress repository
		 * @since   1.0.2
		 * @example https://wordpress.org/support/view/plugin-reviews/compi
		 * @access  public
		 * @var     string
		 */
		public $wp_plugin_review_url = 'https://wordpress.org/support/view/plugin-reviews/compi?filter=5#postform';
		 ...
		 * @var    string
		 */
		public $github_repo_url = "https://github.com/wpdots/compi/";
		 ...
		 * @var    string
		 */
		public $transifex_project_url = "https://www.transifex.com/projects/p/wpdots/";
		 ...
		 * @var    string
		 */
		public $transifex_project_slug = 'wpdots';
		 ...
		 * @var    string
		 */
		public $menu_name = "Compi";

		/**
		 * Compi title page name.
		 *
		 * @since  1.0.0
		 * @access public
		 * @var    string
		 */
		public $title_name = "Compi Settings - Make the best WordPress theme even better.";

		/**
		 * Manage Plugin.
		 *
		 *         the user must have to control Compi.
		 * @since  1.0.0
		 * @access public
		 * @var    string
		 */
		 ...
		 * @var    string
		 */
		public $twitter_username = "wpdots";
		 ...
		 * Main Compi Instance
		 *
		 * Ensures only one instance of Compi is loaded or can be loaded.
		 *
		 * @since  1.0.0
...
*@param  mixed $links
		...
		/**
		 * Auto-load Compi classes on demand to reduce memory consumption.
		 *
		 * @since  1.0.0
		 * @access public
		 * @param  mixed $class
		...
		private function define_constants() {
			...
			if ( !defined( 'COMPI' ) ) {
				define( 'COMPI', $this->name );
			}
			if ( !defined( 'COMPI_FILE' ) ) {
				define( 'COMPI_FILE', __FILE__ );
			}
			if ( !defined( 'COMPI_VERSION' ) ) {
				define( 'COMPI_VERSION', $this->version );
			}
			if ( !defined( 'COMPI_WP_VERSION_REQUIRE' ) ) {
				define( 'COMPI_WP_VERSION_REQUIRE', $this->wp_version_min );
			}
			if ( !defined( 'COMPI_MENU_NAME' ) ) {
				define( 'COMPI_MENU_NAME', strtolower( str_replace( ' ', '-', $this->menu_name ) ) );
			}
			if ( !defined( 'COMPI_PAGE' ) ) {
				define( 'COMPI_PAGE', str_replace( '_', '-', $this->plugin_slug ) );
			}
			if ( !defined( 'COMPI_SCREEN_ID' ) ) {
				define( 'COMPI_SCREEN_ID', strtolower( str_replace( ' ', '-', COMPI_PAGE ) ) );
			}
			if ( !defined( 'COMPI_SLUG' ) ) {
				define( 'COMPI_SLUG', $this->plugin_slug );
			}
			if ( !defined( 'COMPI_TEXT_DOMAIN' ) ) {
				define( 'COMPI_TEXT_DOMAIN', $this->text_domain );
			}
			if ( !defined( 'COMPI_DEFAULT_SETTINGS_TAB' ) ) {
				define( 'COMPI_DEFAULT_SETTINGS_TAB', 'tab_one' );
			}

			if ( !defined( 'COMPI_README_FILE' ) ) {
				define( 'COMPI_README_FILE', 'http://plugins.svn.wordpress.org/compi/trunk/readme.txt' );
			}

			if ( !defined( 'COMPI_GITHUB_REPO_URI' ) ) {
				define( 'COMPI_GITHUB_REPO_URI', $this->github_repo_url );
			}
			if ( !defined( 'COMPI_TRANSIFEX_PROJECT_URI' ) ) {
				define( 'COMPI_TRANSIFEX_PROJECT_URI', $this->transifex_project_url );
			}
		...
		/**
		 * Checks that the WordPress setup meets Compi requirements.
		 *
		 * @since  1.0.0
		 * @access private
		...
		/**
		 * Runs when Compi is initialized.
		 *
		 * @since  1.0.0
		 * @access public
		...
		public function init_compi() {
			...
			if ( !is_admin() || defined( 'DOING_AJAX' ) ) {
				 ...
				 */
				$ssl_filters = apply_filters( 'compi_force_ssl_filter', array(
					'post_thumbnail_html',
					'widget_text',
...
*@param  mixed $content
		...
		private function register_scripts_and_styles() {
			...
			if ( is_admin() ) {
				...
				$this->load_file( $this->plugin_slug . '_admin_script', '/assets/js/admin/compi' . COMPI_SCRIPT_MODE . '.js', true, array(
					'jquery',
					'jquery-blockui',
					'jquery-ui-sortable',
					'jquery-ui-widget',
				...
				// Chosen
				$this->load_file( 'ajax-chosen', '/assets/js/chosen/ajax-chosen.jquery' . COMPI_SCRIPT_MODE . '.js', true, array(
					'jquery',
					'chosen'
				), $this->version );
		 ...
		 * @param  string $version Optional, can match the version of Compi or version of the source file.
		 * @global string $wp_version
		 */
